...
#ifndef __KM_MENU_H__
#define __KM_MENU_H__
...
typedef const char *(*vkb_query_fn_t)(const char *replace, consvar_t *cvar);
extern struct menutyping_s
{
	...
	consvar_t *cvar; // STAR: command to edit //
	boolean (*closeroutine)(void); // STAR: function to run upon closing, overrides normal closeroutine if true //
	void (*abortroutine)(void); // STAR: function to run upon full close //
} menutyping;
