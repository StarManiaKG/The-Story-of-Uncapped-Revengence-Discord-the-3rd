...
void M_OpenVirtualKeyboard(size_t cachelen, vkb_query_fn_t queryfn, menu_t *dummymenu, consvar_t *cvar, boolean (*closeroutine)(void), void (*abortroutine)(void))
{
	menutyping.active = true;
	menutyping.menutypingclose = false;

	menutyping.cvar = cvar;
	menutyping.closeroutine = closeroutine;
	menutyping.abortroutine = abortroutine;
	...
	strlcpy(menutyping.cache, queryfn(NULL, cvar), cachelen + 1);
...
static void M_CloseVirtualKeyboard(void)
{
	...
	if (menutyping.closeroutine && !menutyping.closeroutine())
		menutyping.queryfn(menutyping.cache, menutyping.cvar);
}
...
void M_AbortVirtualKeyboard(void)
{
	...
	if (menutyping.abortroutine)
		menutyping.abortroutine();
	...
	menutyping.cvar = NULL;
	menutyping.closeroutine = NULL;
	menutyping.abortroutine = NULL;
}
