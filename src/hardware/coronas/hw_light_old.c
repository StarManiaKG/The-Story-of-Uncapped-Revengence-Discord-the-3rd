...
#define LIGHT_X(i) dynlights->x[(i)]
#define LIGHT_Y(i) dynlights->y[(i)]
#define LIGHT_Z(i) dynlights->z[(i)]
...
{
	fixed_t p1z = FLOAT_TO_FIXED(p1->z), p2z = FLOAT_TO_FIXED(p2->z);
	fixed_t p1x = FLOAT_TO_FIXED(p1->x), p2x = FLOAT_TO_FIXED(p2->x);
...
void HWR_WallLighting(FOutVector *wlVerts)
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		d[2] = LIGHT_X(j) - wlVerts[0].x;
		d[3] = LIGHT_Z(j) - wlVerts[0].z;
		// backface cull
		...
		//dAB = (float)sqrtf((wlVerts[0].x-wlVerts[2].x)*(wlVerts[0].x-wlVerts[2].x)+(wlVerts[0].z-wlVerts[2].z)*(wlVerts[0].z-wlVerts[2].z));
		...
		for (i = 0; i < 4; i++)
		{
			...
			wlVerts[i].t = (float)(0.5f + (wlVerts[i].y-LIGHT_Y(j))*s*1.2f);
		}
...
void HWR_PlaneLighting(FOutVector *clVerts, int nrClipVerts)
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		if ((clVerts[0].y > atransform.z)       // true mean it is a ceiling false is a floor
			^ (LIGHT_Y(j) < clVerts[0].y)) // true mean light is down plane false light is up plane
			continue;
		dist_p2d = (clVerts[0].y-LIGHT_Y(j));
		dist_p2d *= dist_p2d;
		...
		{
			clVerts[i].s = 0.5f + (clVerts[i].x-LIGHT_X(j))*s;
			clVerts[i].t = 0.5f + (clVerts[i].z-LIGHT_Z(j))*s*1.2f;
		}
...
////////////////
#ifdef STAR_CORONAS
void HWR_DrawCoronas(FRenderState& state)
{
...
void HWR_DrawCoronas(void)
{
	...
	for (j = 0;j < dynlights->nb;j++)
	{
		...
		float           cx = LIGHT_X(j);
		float           cy = LIGHT_Y(j);
		float           cz = LIGHT_Z(j); // gravity center
#endif
		double          size = 0.0f;
		...
		HWD.pfnDrawPolygon(&Surf, light, 4, LIGHTMAPFLAGS | PF_NoDepthTest | PF_Corona);
	}
}
...
void HWR_DL_AddLight(gl_vissprite_t *spr, GLPatch_t *patch)
{
	...
	double cx = FIXED_TO_FLOAT(spr->mobj->x);
	double cz = FIXED_TO_FLOAT(spr->mobj->z)+FIXED_TO_FLOAT(spr->mobj->height>>1);//+p_lspr->light_yoffset;
	double cy = FIXED_TO_FLOAT(spr->mobj->y);

	LIGHT_POS(dynlights->nb).x = cx;
	LIGHT_POS(dynlights->nb).y = cz;
	LIGHT_POS(dynlights->nb).z = cy;

	LIGHT_X(dynlights->nb) = cx;
	LIGHT_Y(dynlights->nb) = cz;
	LIGHT_Z(dynlights->nb) = cy;

#if 0
	STAR_CONS_Printf(STAR_CONS_TSOURDT3RD_DEBUG, M_GetText(
		"THING\n"
		"%d,%d,%d\n\n"
	...
	spr->mobj->x, spr->mobj->y, spr->mobj->z,
	LIGHT_POS(dynlights->nb).x, LIGHT_POS(dynlights->nb).y, LIGHT_POS(dynlights->nb).z,
	dynlights->x, dynlights->y, dynlights->z,
	cx, cy, cz);
#endif
...
static void HWR_SetLight(void)
{
	...
	if (!lightmappatch->mipmap->downloaded && !lightmappatch->mipmap->data)
	{
		...
		//lightmappatch->mipmap->flags = TF_WRAPXY; // DEBUG: view the overdraw !
#endif	
	}
...
{
	double cx, cz, cy;
	light_t *p_lspr = t_lspr[thing->sprite];
	...
	cx = FIXED_TO_FLOAT(thing->x);
	cz = FIXED_TO_FLOAT(thing->z)+FIXED_TO_FLOAT(thing->height>>1);//+p_lspr->light_yoffset;
	cy = FIXED_TO_FLOAT(thing->y);
