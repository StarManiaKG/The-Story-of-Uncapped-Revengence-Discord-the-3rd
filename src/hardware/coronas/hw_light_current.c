...
light_t *t_lspr[NUMSPRITES] =
{
	...
	&lspr[SUPERSONIC_L/*BLUESHINE_L*/], // SPR_CEMG
...
#if 0
void HWR_DrawCoronas(double x, double y, double z, UINT32 color, double size)
{	
	FOutVector      light[4];
	//wallVert3D light[4];
	FSurfaceInfo    Surf;
	...
	HWD.pfnDrawPolygon(&Surf, light, 4, LIGHTMAPFLAGS);
...
void HWR_DrawCoronas(double x, double y, double z, double size)
{
	...
	for (j = 0;j < dynlights->nb;j++)
	{
		...
		if (cz >= 255*8+250)
			continue;
		Surf.PolyColor.rgba = p_lspr->corona_color; //color;
		if (cz > 250.0f)
...
#ifdef DL_HIGH_QUALITY
		Surf.PolyColor.s.alpha = (UINT8/*byte*/)((1-dist_p2d/DL_SQRRADIUS(j))*Surf.PolyColor.s.alpha);
#endif
...
// SRB2CBTODO: The s and t of the texture should be the same as the plane/wall
void HWR_PlaneLighting(FOutVector *clVerts, int nrClipVerts)
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		Surf.PolyColor.rgba = LONG(dynlights->p_lspr[j]->dynamic_color);
#ifdef DL_HIGH_QUALITY
		Surf.PolyColor.s.alpha = (unsigned char/*byte*/)((1 - dist_p2d/DL_SQRRADIUS(j))*Surf.PolyColor.s.alpha);
#endif
...
void HWR_SpriteLighting(FOutVector *wlVerts) // SRB2CBTODO: Support sprites to be lit too
{
	...
	for (j = 0; j < dynlights->nb; j++)
	{
		...
		Surf.PolyColor.s.alpha = (byte)((1-dist_p2d/DL_SQRRADIUS(j))*Surf.PolyColor.s.alpha);
...
void HWR_RenderFloorSplat(FOutVector *clVerts, int nrClipVerts)
{
	...
	for (j = 0; j < dynshadows->nb; j++)
	{
		...
		for (i = 0; i < nrClipVerts; i++)
		{
			clVerts[i].s = (clVerts[i].x - SHADOW_POS(j).x)*s;
			clVerts[i].t = (clVerts[i].z - SHADOW_POS(j).z)*s;
		}
		...
		Surf.PolyColor.s.alpha = (byte)((1 - dist_p2d/DS_SQRRADIUS(j))*128); // 128 = max darkness of shadow
		
		HWD.pfnDrawPolygon(&Surf, clVerts, nrClipVerts, PF_Modulated|PF_Decal|PF_Translucent);
...
{	
	light_t   *p_lspr;
	//light_t   *newlight newlight = Z_Malloc(sizeof(light_t), PU_LEVEL, NULL);
	...
	// check if sprite contain dynamic light
	//newlight->dynamic_color = LONG(dynlights->p_lspr[dynlights->nb]->dynamic_color);
	//newlight->dynamic_radius = radius;
	//newlight->light_xoffset = newlight->light_yoffset = 0.0f; // SRB2CBTODO: Could be used?

	//dynlights->light_xoffset = dynlights->light_yoffset = 0.0f; // SRB2CBTODO: Could be used?
	p_lspr->light_xoffset = p_lspr->light_yoffset = 0.0f;
...
void HWR_WallShading(FOutVector *wlVerts)
{
	...
	for (j = 0; j < dynshadows->nb; j++)
	{
		...
		HWD.pfnDrawPolygon (&Surf, wlVerts, 4, PF_Modulated|PF_Decal|PF_Translucent);
...
void HWR_AddDynShadow(double x, double y, double z, ULONG color, byte radius, gl_vissprite_t *spr)
{
	...
	if (dynshadows->nb < DL_MAX_SHADOW)
	{
		...
		//P_SetTarget(&dynshadows->p_sspr[dynshadows->nb], spr->mobj);
